...
def sync():
    ...
    try:
        ...
        # ⚡ CHUNK SYNC LOGIC: ဒေတာ ဝန်မပိစေရန် ပစ္စည်း ၄၀ စီစီစီသာ ခွဲပို့မည့်စနစ်
        CHUNK_SIZE = 40
        ...
        for i in range(0, total_products, CHUNK_SIZE):
            ...
            for p in chunk_products:
                ...
                p_image = p.get("image_128", "") # ⚡ Base64 ဓာတ်ပုံစာသားကို Sheet ထဲ တိုက်ရိုက်သွင်းသည်
                
                categ_data = p.get("categ_id", False)
                p_category = [item.strip() for item in categ_data[1].split("/")][-1] if categ_data else "Uncategorized"
                
                # Google Sheet Layout အတိုင်း တန်းစီသည်: [ID, Name, Myanmar_Name, Price, Image, Category]
                raw_data_rows.append([p_id, p_name_en, "", p_price, p_image, p_category])
            
            # ပထမဆုံး Chunk ဆိုလျှင် Sheet အဟောင်းကို ရှင်းထုတ်ခိုင်းမည်
            ...
            time.sleep(0.8) # Google Rate Limit မမိစေရန် ခဏစောင့်ပေးခြင်း
